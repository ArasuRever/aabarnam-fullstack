...
import { Users, Mail, Phone, MapPin, Package, Heart, X, Calendar } from 'lucide-react';

const Customers = () => {
  const [customers, setCustomers] = useState([]);
  const [loading, setLoading] = useState(true);
  const [selectedUser, setSelectedUser] = useState(null); // Holds full details
  ...
  return (
    <div className="p-4 sm:p-8 animate-fade-in relative">
              ...
              <div className="p-6 flex-1 space-y-8">
                 ...
                 <div className="flex items-center gap-4">
                    <div className="w-16 h-16 bg-gold text-black rounded-full flex items-center justify-center text-2xl font-bold">
                       {selectedUser.profile.name.charAt(0)}
                    </div>
                    <div>
                          ...
                          {selectedUser.orders.map(order => (
                             <div key={order.id} className="bg-gray-50 p-3 rounded-lg border border-gray-100 flex justify-between items-center text-sm">
                                ...
                                <div className="text-right">
                                   ...
                                   <p className={`text-[10px] font-bold ${order.status === 'DELIVERED' ? 'text-green-600' : 'text-blue-600'}`}>{order.status}</p>
                                </div>
                             </div>
                          ))}
                       </div>
      ...
      <div className="mb-8">
        <h1 className="text-2xl font-bold flex items-center gap-2 text-gray-900"><Users className="text-gold" /> Customer Directory</h1>
      </div>
              ...
              <tr key={user.id} className="hover:bg-gray-50 transition">
                <td className="p-4">
                   <div className="flex items-center gap-3">
                      <div className="w-8 h-8 bg-gold/20 text-gold-dark rounded-full flex items-center justify-center font-bold text-xs">
                         {user.name.charAt(0)}
                      </div>
                ...
                <td className="p-4">
                   <button onClick={() => viewCustomerDetails(user.id)} className="text-xs font-bold text-blue-600 bg-blue-50 px-3 py-1.5 rounded-lg hover:bg-blue-100 transition">
                      View Profile
                   </button>
                </td>
              </tr>
