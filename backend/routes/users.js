...
router.get('/:id', async (req, res) => {
    ...
    try {
        ...
        const ordersRes = await pool.query(`SELECT o.*, COALESCE(json_agg(json_build_object('product_name', oi.product_name, 'quantity', oi.quantity, 'price', oi.price_at_purchase)) FILTER (WHERE oi.id IS NOT NULL), '[]') as items FROM orders o LEFT JOIN order_items oi ON o.id = oi.order_id WHERE o.user_id = $1 GROUP BY o.id ORDER BY o.created_at DESC`, [userId]);
        const wishlistRes = await pool.query(`SELECT p.id, p.name, p.sku, p.item_type, w.created_at as added_on FROM wishlists w JOIN products p ON w.product_id = p.id WHERE w.user_id = $1`, [userId]);

        res.json({ profile: userRes.rows[0], addresses: addressesRes.rows, orders: ordersRes.rows, wishlist: wishlistRes.rows });
