...
router.post('/', async (req, res) => {
    ...
    try {
        ...
        // We recalculate the absolute floor price for every item 
        // to ensure the frontend wasn't hacked.
        // ---------------------------------------------------------
        for (const item of items) {
            ...
            // Allow a tiny 1% buffer below the 5% margin in case live rates fluctuated during checkout
            const secureFloorPrice = wholesaleCost * 1.04; 

            if (parseFloat(item.price_breakdown.final_total_price) < secureFloorPrice) {
                console.error(`🚨 SECURITY ALERT: Tampering detected! User attempted to buy ${dbItem.name} for ₹${item.price_breakdown.final_total_price}, which is below floor ₹${secureFloorPrice}`);
        ...
        for (const item of items) {
            await client.query(
                ...
                [orderId, item.id, item.name, item.quantity || 1, item.price_breakdown.final_total_price]
            );

            // Decrease stock
            await client.query('UPDATE products SET stock_quantity = stock_quantity - $1 WHERE id = $2', [item.quantity || 1, item.id]);
        }
