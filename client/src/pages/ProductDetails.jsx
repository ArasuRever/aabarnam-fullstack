...
import { ShoppingBag, ShieldCheck, Truck, RotateCcw, Heart, MapPin, Info, X } from 'lucide-react';
          ...
          <div className="flex flex-col h-full">
            ...
            <div className="bg-gray-50 p-6 rounded-xl border border-gray-100 mb-6">
              <div className="flex items-end gap-3 mb-2">
                ...
                <span className="text-[10px] text-green-600 font-bold mb-2 bg-green-100 px-2 py-1 rounded-full flex items-center gap-1">
                    <span className="w-1.5 h-1.5 bg-green-500 rounded-full animate-pulse"></span> Live Rate
                </span>
              </div>
              <button 
                 onClick={() => setShowBreakdown(true)}
                 className="text-xs text-gold-dark font-bold hover:underline flex items-center gap-1 cursor-pointer"
              >
            ...
            <div className="flex gap-4 mb-10">
              <button 
                ...
                className="flex-1 bg-black text-white py-4 rounded-xl font-bold text-lg hover:bg-gray-800 transition-all flex items-center justify-center gap-2 shadow-lg active:scale-95 group"
              >
                <ShoppingBag size={20} className="group-hover:text-gold transition-colors" /> Add to Cart
              </button>
              <button 
                onClick={() => toast('Added to your Wishlist ❤️', { icon: '✨' })}
                className="w-14 h-14 border border-gray-200 rounded-xl flex items-center justify-center text-gray-400 hover:text-red-500 hover:border-red-200 hover:bg-red-50 transition-all active:scale-95"
              >
                 <Heart size={20} />
              </button>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-center border-t border-gray-100 pt-8">
               <div className="flex flex-col items-center gap-2">
                  <div className="bg-gray-50 p-3 rounded-full"><ShieldCheck size={20} className="text-gold" /></div>
                  <span className="text-[10px] font-bold text-gray-600 uppercase tracking-wide">BIS Hallmarked</span>
               </div>
               <div className="flex flex-col items-center gap-2">
                  <div className="bg-gray-50 p-3 rounded-full"><Truck size={20} className="text-gold" /></div>
                  <span className="text-[10px] font-bold text-gray-600 uppercase tracking-wide">Insured Shipping</span>
               </div>
               <div className="flex flex-col items-center gap-2">
                  <div className="bg-gray-50 p-3 rounded-full"><RotateCcw size={20} className="text-gold" /></div>
